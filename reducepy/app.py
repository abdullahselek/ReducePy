...
class MainHandler(tornado.web.RequestHandler):
    ...
    def background_task(self, url: str):
        if url:
            ...
            response = {"error": True, "message": "Please post a valid url"}
            return 400, json.dumps(response, sort_keys=True)
        else:
            response = {"error": True, "message": "Please post a url"}
        return 400, json.dumps(response, sort_keys=True)
    ...
    def get(self, *args):
        if len(args) == 1:
            ...
            if url:
                ...
                response = {"error": True, "message": "No url found with given unique"}
                self.set_status(404)
                return self.write(json.dumps(response, sort_keys=True))
        else:
            response = {"error": True, "message": "Please set only one argument path"}
            self.set_status(400)
            return self.write(json.dumps(response, sort_keys=True))
